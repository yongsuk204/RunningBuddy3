...
/*
 * Hashing Methods
 * - hashSecurityAnswer(): 보안질문 답변을 pepper + salt와 함께 SHA-512로 해시화
 * - verifySecurityAnswer(): 입력된 답변과 저장된 해시값 비교 검증
 * - hashEmail(): 이메일 전체를 pepper + salt와 함께 SHA-512로 해시화
 *
 * Private Methods
...
class SecurityService {
    ...
    // Purpose: 전화번호를 해시화 (publicdata 컬렉션 저장용)
    func hashPhoneNumber(_ phoneNumber: String) -> String {
        ...
        // Step 2: pepper와 salt를 결합한 문자열 생성
        let combinedString = normalizedNumber + Config.pepper + Config.salt
